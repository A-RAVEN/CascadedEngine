...
[VK_FORMAT_FEATURE_2_STORAGE_READ_WITHOUT_FORMAT_BIT](VkFormatFeatureFlagBits2.html)
 ...
 VUID-vkCmdDraw-None-08600
   ...
   at the same pipeline bind point, with layouts compatible for set *n*
   with
the layout(s) used to create the shader, as described in
[Pipeline Layout Compatibility](../../../../spec/latest/chapters/descriptorsets.html#descriptors-compatibility)

 VUID-vkCmdDraw-None-08601

   If a [a bound shader](../../../../spec/latest/chapters/shaders.html#shaders-binding)
   was created with a pipeline layout
or specified push constant ranges
and
   statically uses a push constant value, that value must have been set
   for the same pipeline bind point, with push constant ranges that are
   [compatible](../../../../spec/latest/chapters/descriptorsets.html#descriptors-compatibility) with the push constant range
...
or [VK_FORMAT_FEATURE_2_BLOCK_MATCHING_SXD_BIT_QCOM](VkFormatFeatureFlagBits2.html)
...
read from a reference image as result of this command,
and the
[blockMatchExtendedClampToEdge](../../../../spec/latest/chapters/features.html#features-blockMatchExtendedClampToEdge)
feature is not enabled or the *input* sampler was not created with both
`addressModeU` and `addressModeV` equal to
[VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE](VkSamplerAddressMode.html)
...
If a `OpImageBlockMatchWindow*QCOM` or
`OpImageBlockMatchGather*QCOM` instruction uses a [VkSampler](VkSampler.html) as
a result of this command,
and the
[`blockMatchExtendedClampToEdge`](../../../../spec/latest/chapters/features.html#features-blockMatchExtendedClampToEdge)
feature is not enabled,
the *input* sampler must not have been created with either
...
If a `OpImageBlockMatchWindow*QCOM` or
`OpImageBlockMatchGather*QCOM` instruction uses a [VkSampler](VkSampler.html) as
a result of this command, and the *input* sampler was created with
either `addressModeU` or `addressModeV` equal to
[VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE](VkSamplerAddressMode.html), then the sampler must have
been created with both `addressModeU` and `addressModeV` equal
to [VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE](VkSamplerAddressMode.html)
...
If an acceleration structure is accessed as a result of this command
through an `OpRayQueryProceedKHR` operation
, the `[VK_EXT_opacity_micromap](VK_EXT_opacity_micromap.html)` extension is not enabled,
and the acceleration structure is built using geometry with a
[VkAccelerationStructureTrianglesOpacityMicromapKHR](VkAccelerationStructureTrianglesOpacityMicromapKHR.html) structure
included in its `pNext` chain, the shader must have specified a
`OpacityMicromapIdKHR` execution mode with its specialization
constant operand equal to [VK_TRUE](VK_TRUE.html)

 VUID-vkCmdDraw-micromap-11637

If an acceleration structure is accessed as a result of this command and
the acceleration structure was constructed through deserialization, all
micromap arrays referenced by the acceleration structure that were not
replaced by an acceleration structure update command must have been
deserialized using the serialized data of the corresponding micromaps
used to originally build the acceleration structure prior to the
execution of this command on the device

 VUID-vkCmdDraw-micromap-11638

If an acceleration structure is accessed as a result of this command and
the acceleration structure is built using geometry with an
`indexBuffer` containing both [VkOpacityMicromapSpecialIndexKHR](VkOpacityMicromapSpecialIndexKHR.html)
values and indices into an associated micromap array, any
[VkPipeline](VkPipeline.html) bound to the pipeline bind point used by this command
...
[VK_PIPELINE_CREATE_2_OPACITY_MICROMAP_DISALLOW_MIXED_SPECIAL_INDEX_BIT_KHR](VkPipelineCreateFlagBits2.html)
 ...
 VUID-vkCmdDraw-micromap-11639

If an acceleration structure is accessed as a result of this command and
the acceleration structure is built using geometry with an
`indexBuffer` containing both [VkOpacityMicromapSpecialIndexKHR](VkOpacityMicromapSpecialIndexKHR.html)
values and indices into an associated micromap array, any
[VkShaderEXT](VkShaderEXT.html) that accessed the descriptor must not have been
...
[bound tile memory object](../../../../spec/latest/chapters/memory.html#memory-bind-tile-memory) in
`commandBuffer`
...
[VkTensorViewARM](VkTensorViewARM.html) used for the access
 ...
 VUID-vkCmdDraw-OpTensorReadARM-12462

If a [VK_DESCRIPTOR_TYPE_TENSOR_ARM](VkDescriptorType.html) descriptor is accessed as a
result of this command, using `OpTensorReadARM` or
`OpTensorWriteARM`, and the underlying [VkTensorARM](VkTensorARM.html) object was
created with a [VK_TENSOR_TILING_BRICK_16_WIDE_ARM](VkTensorTilingARM.html),
[VK_TENSOR_TILING_BRICK_8_WIDE_ARM](VkTensorTilingARM.html),
[VK_TENSOR_TILING_BRICK_4_WIDE_ARM](VkTensorTilingARM.html),
[VK_TENSOR_TILING_BLOCK_U_INTERLEAVED_ARM](VkTensorTilingARM.html), or
[VK_TENSOR_TILING_BLOCK_U_INTERLEAVED_64K_ARM](VkTensorTilingARM.html)
[VkTensorTilingARM](VkTensorTilingARM.html), and the object read or written is an array,
then the innermost coordinate provided to `OpTensorReadARM` or
`OpTensorWriteARM` via the `Coordinates` operand must be a
multiple of the largest power of 2 that is less than or equal to the
number of components of the array

 VUID-vkCmdDraw-pNext-12463

If a [VK_DESCRIPTOR_TYPE_TENSOR_ARM](VkDescriptorType.html) descriptor is accessed as a
result of this command and the corresponding [VkTensorARM](VkTensorARM.html) object
was created with a [VkTensorRollingBackingCreateInfoARM](VkTensorRollingBackingCreateInfoARM.html) structure
in the `pNext` chain of [VkTensorCreateInfoARM](VkTensorCreateInfoARM.html), then the number
of elements accessed with one `OpTensorReadARM` or
`OpTensorWriteARM`, must be less than or equal to the number of
elements available from the starting coordinate in the rolling backing
along the innermost dimension, defined as
`wraps`[`dimensionCount` - 1] - InnermostCoordinate %
`wraps`[`dimensionCount` - 1] where InnermostCoordinate is the
last component of the coordinates array provided via the
`Coordinates` operand

 VUID-vkCmdDraw-None-12472

Descriptors dynamically accessed in bound descriptor heaps must be
valid
...
[VK_DESCRIPTOR_MAPPING_SOURCE_RESOURCE_HEAP_DATA_EXT](VkDescriptorMappingSourceEXT.html) and a shader
accesses a uniform buffer using that mapping, the address that the
...
[effective rasterization input    topology](../../../../spec/latest/chapters/drawing.html#drawing-rasterization-input-topology) is in the line topology class, then [vkCmdSetLineWidth](vkCmdSetLineWidth.html)
...
[VK_DYNAMIC_STATE_COLOR_BLEND_ENABLE_EXT](VkDynamicState.html) dynamic state enabled, the
current render pass instance was begun with [vkCmdBeginRendering](vkCmdBeginRendering.html),
and it includes a color attachment that uses the
[VK_RESOLVE_MODE_EXTERNAL_FORMAT_DOWNSAMPLE_BIT_ANDROID](VkResolveModeFlagBits.html) resolve
mode, then the last call to [vkCmdSetColorBlendEnableEXT](vkCmdSetColorBlendEnableEXT.html) must have
set the blend enable to [VK_FALSE](VK_FALSE.html) prior to this drawing command
...
the current render pass instance was begun with
[vkCmdBeginRendering](vkCmdBeginRendering.html), and it includes a color attachment that uses
the [VK_RESOLVE_MODE_EXTERNAL_FORMAT_DOWNSAMPLE_BIT_ANDROID](VkResolveModeFlagBits.html) resolve
mode, then the last call to [vkCmdSetRasterizationSamplesEXT](vkCmdSetRasterizationSamplesEXT.html) must
have set `rasterizationSamples` to [VK_SAMPLE_COUNT_1_BIT](VkSampleCountFlagBits.html) prior
...
[vkCmdBeginRendering](vkCmdBeginRendering.html), and it includes a color attachment that uses
the [VK_RESOLVE_MODE_EXTERNAL_FORMAT_DOWNSAMPLE_BIT_ANDROID](VkResolveModeFlagBits.html) resolve
mode, then the last call to [vkCmdSetFragmentShadingRateKHR](vkCmdSetFragmentShadingRateKHR.html) must
have set `pFragmentSize->width` to `1` prior to this drawing command
...
the [VK_RESOLVE_MODE_EXTERNAL_FORMAT_DOWNSAMPLE_BIT_ANDROID](VkResolveModeFlagBits.html) resolve
mode, then the last call to [vkCmdSetFragmentShadingRateKHR](vkCmdSetFragmentShadingRateKHR.html) must
have set `pFragmentSize->height` to `1` prior to this drawing
command
...
[effective rasterization input    topology](../../../../spec/latest/chapters/drawing.html#drawing-rasterization-input-topology) is in the line or point topology class, then the
[current value](../../../../spec/latest/chapters/pipelines.html#dynamic-state-current-value) of
...
or
    ...
    and the [current value](../../../../spec/latest/chapters/pipelines.html#dynamic-state-current-value) of
    `rasterizerDiscardEnable` is [VK_FALSE](VK_FALSE.html), and the
    [effective rasterization input    topology](../../../../spec/latest/chapters/drawing.html#drawing-rasterization-input-topology) is in the line topology class, then
...
a graphics pipeline is bound which was created with the
[VK_DYNAMIC_STATE_LINE_RASTERIZATION_MODE_EXT](VkDynamicState.html) dynamic state
enabled, the [effective    rasterization input topology](../../../../spec/latest/chapters/drawing.html#drawing-rasterization-input-topology) is in the line topology class, and the
...
or
    ...
    [VK_COLOR_COMPONENT_G_BIT](VkColorComponentFlagBits.html), [VK_COLOR_COMPONENT_B_BIT](VkColorComponentFlagBits.html), and
    [VK_COLOR_COMPONENT_A_BIT](VkColorComponentFlagBits.html), or none of them
...
value of the format must be [VK_FORMAT_UNDEFINED](VkFormat.html)
 ...
 VUID-vkCmdDraw-colorAttachmentCount-11871
...
pipeline
 ...
 VUID-vkCmdDraw-pDepthAttachment-11872
...
create [VkRenderingInfo](VkRenderingInfo.html)::`pDepthAttachment->resolveImageView`
 ...
 VUID-vkCmdDraw-pStencilAttachment-11873
...
If there are any shader objects bound that were created with `flags`
that includes the [VK_SHADER_CREATE_INDEPENDENT_SETS_BIT_KHR](VkShaderCreateFlagBitsEXT.html) bit,
then all bound shader objects must have been created with the
[VK_SHADER_CREATE_INDEPENDENT_SETS_BIT_KHR](VkShaderCreateFlagBitsEXT.html) bit

 VUID-vkCmdDraw-flags-13362

If there are any shader objects bound that were created with `flags`
that includes the [VK_SHADER_CREATE_INDEPENDENT_SETS_BIT_KHR](VkShaderCreateFlagBitsEXT.html) bit,
then the pipeline layout of the bound descriptor sets must have been
created with [VK_PIPELINE_LAYOUT_CREATE_INDEPENDENT_SETS_BIT_EXT](VkPipelineLayoutCreateFlagBits.html)
set
...
If there are any shader objects bound that were created with `flags`
that includes the [VK_SHADER_CREATE_INDEPENDENT_SETS_BIT_KHR](VkShaderCreateFlagBitsEXT.html) bit,
and any of the descriptor set layouts used to create the pipeline layout
of the bound descriptor sets were created with any binding’s
[VkDescriptorSetLayoutBinding](VkDescriptorSetLayoutBinding.html)::`stageFlags` set with
[VK_SHADER_STAGE_VERTEX_BIT](VkShaderStageFlagBits.html),
[VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT](VkShaderStageFlagBits.html),
[VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT](VkShaderStageFlagBits.html) or
[VK_SHADER_STAGE_GEOMETRY_BIT](VkShaderStageFlagBits.html), then there must be no binding with
[VkDescriptorSetLayoutBinding](VkDescriptorSetLayoutBinding.html)::`stageFlags` set with
...
If there are any shader objects bound that were created with `flags`
that includes both the [VK_SHADER_CREATE_INDEPENDENT_SETS_BIT_KHR](VkShaderCreateFlagBitsEXT.html)
and [VK_SHADER_CREATE_NO_TASK_SHADER_BIT_EXT](VkShaderCreateFlagBitsEXT.html) bits, the pipeline
layout of the bound descriptor sets must have been created with
[VK_PIPELINE_LAYOUT_CREATE_NO_TASK_SHADER_BIT_KHR](VkPipelineLayoutCreateFlagBits.html) set

 VUID-vkCmdDraw-flags-13365

If there are any shader objects bound that were created with `flags`
that includes both the [VK_SHADER_CREATE_INDEPENDENT_SETS_BIT_KHR](VkShaderCreateFlagBitsEXT.html)
and [VK_SHADER_CREATE_NO_TASK_SHADER_BIT_EXT](VkShaderCreateFlagBitsEXT.html) bits, there must be
no shader object bound to the [VK_SHADER_STAGE_TASK_BIT_EXT](VkShaderStageFlagBits.html) stage
...
[VK_PIPELINE_ROBUSTNESS_BUFFER_BEHAVIOR_ROBUST_BUFFER_ACCESS](VkPipelineRobustnessBufferBehavior.html) for
`vertexInputs`,
...
be a multiple of the [component size of the `format`](../../../../spec/latest/chapters/formats.html#formats)
 ...
 VUID-vkCmdDraw-None-07842
    ...
    If
    there is a shader object bound to the [VK_SHADER_STAGE_VERTEX_BIT](VkShaderStageFlagBits.html)
    stage
or
    the bound graphics pipeline state was created with the
    [VK_DYNAMIC_STATE_PRIMITIVE_TOPOLOGY](VkDynamicState.html) dynamic state enabled
...
If the bound graphics pipeline state was created with the
[VK_DYNAMIC_STATE_PRIMITIVE_TOPOLOGY](VkDynamicState.html) dynamic state enabled
and the [    `dynamicPrimitiveTopologyUnrestricted`](../../../../spec/latest/chapters/limits.html#limits-dynamicPrimitiveTopologyUnrestricted) is [VK_FALSE](VK_FALSE.html),
...
If a [VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT](VkShaderStageFlagBits.html) stage is bound, then
the [current value](../../../../spec/latest/chapters/pipelines.html#dynamic-state-current-value) of
`primitiveTopology` must be [VK_PRIMITIVE_TOPOLOGY_PATCH_LIST](VkPrimitiveTopology.html)
prior to this drawing command
...
[VK_PRIMITIVE_TOPOLOGY_PATCH_LIST](VkPrimitiveTopology.html) prior to this drawing command,
then a [VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT](VkShaderStageFlagBits.html) stage must be
bound

 VUID-vkCmdDraw-primitiveTopology-10748

If [vkCmdSetPrimitiveTopology](vkCmdSetPrimitiveTopology.html) set `primitiveTopology` to
[VK_PRIMITIVE_TOPOLOGY_POINT_LIST](VkPrimitiveTopology.html) prior to this drawing command,
the [`maintenance5`](../../../../spec/latest/chapters/features.html#features-maintenance5) feature is not
enabled,
both a [VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT](VkShaderStageFlagBits.html) and
[VK_SHADER_STAGE_GEOMETRY_BIT](VkShaderStageFlagBits.html) stage are not bound, then the
...
[VK_DYNAMIC_STATE_VERTEX_INPUT_BINDING_STRIDE](VkDynamicState.html) dynamic state
enabled,
but without the [VK_DYNAMIC_STATE_VERTEX_INPUT_EXT](VkDynamicState.html) dynamic state
enabled,
then [vkCmdBindVertexBuffers2](vkCmdBindVertexBuffers2.html) with a non-`NULL` `pStrides`
parameter
or [vkCmdBindVertexBuffers3KHR](vkCmdBindVertexBuffers3KHR.html) with `setStride` set to
[VK_TRUE](VK_TRUE.html)
must have been called and not subsequently [    invalidated](../../../../spec/latest/chapters/pipelines.html#dynamic-state-lifetime) in the current command buffer prior to this draw command
...
[VK_DYNAMIC_STATE_VERTEX_INPUT_BINDING_STRIDE](VkDynamicState.html) dynamic state
enabled,
without the [VK_DYNAMIC_STATE_VERTEX_INPUT_EXT](VkDynamicState.html) dynamic state
enabled,
and [vkCmdBindVertexBuffers3KHR](vkCmdBindVertexBuffers3KHR.html) was called and not subsequently
[invalidated](../../../../spec/latest/chapters/pipelines.html#dynamic-state-lifetime) in the current command buffer
prior to this draw command, the value of `setStride` in each of its
`pBindInfos` elements must have been [VK_FALSE](VK_FALSE.html)
...
or
 ...
 VUID-vkCmdDraw-None-04875
    ...
    [VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT](VkShaderStageFlagBits.html) stage
or
    the bound graphics pipeline state was created with both a
    [VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT](VkShaderStageFlagBits.html) stage and the
    [VK_DYNAMIC_STATE_PATCH_CONTROL_POINTS_EXT](VkDynamicState.html) dynamic state enabled,
    and the [current value](../../../../spec/latest/chapters/pipelines.html#dynamic-state-current-value) of
    `primitiveTopology` is [VK_PRIMITIVE_TOPOLOGY_PATCH_LIST](VkPrimitiveTopology.html), then
...
or
    the bound graphics pipeline state was created with the
    [VK_DYNAMIC_STATE_PRIMITIVE_RESTART_ENABLE](VkDynamicState.html) dynamic state enabled
...
or
    the bound graphics pipeline state was created with the
    [VK_DYNAMIC_STATE_PRIMITIVE_RESTART_ENABLE](VkDynamicState.html) dynamic state enabled,
    then [vkCmdSetPrimitiveRestartEnable](vkCmdSetPrimitiveRestartEnable.html) must be [VK_FALSE](VK_FALSE.html)
...
[VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT](VkShaderStageFlagBits.html) stage or
a graphics pipeline is bound which was created with both a
[VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT](VkShaderStageFlagBits.html) stage and the
[VK_DYNAMIC_STATE_TESSELLATION_DOMAIN_ORIGIN_EXT](VkDynamicState.html) dynamic state
...
[VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT](VkShaderStageFlagBits.html) stages, and if both
...
[Command Buffer Levels](../../../../spec/latest/chapters/cmdbuffers.html#VkCommandBufferLevel)

[Render Pass Scope](../../../../spec/latest/chapters/renderpass.html#vkCmdBeginRenderPass)

[Video Coding Scope](../../../../spec/latest/chapters/videocoding.html#vkCmdBeginVideoCodingKHR)

[Supported Queue Types](../../../../spec/latest/chapters/devsandqueues.html#VkQueueFlagBits)

[Command Type](../../../../spec/latest/chapters/fundamentals.html#fundamentals-queueoperation-command-types)
...
vkCmdDraw is affected by [conditional rendering](../../../../spec/latest/chapters/drawing.html#drawing-conditional-rendering)
...
For more information, see the [Vulkan Specification](../../../../spec/latest/chapters/drawing.html#vkCmdDraw).
