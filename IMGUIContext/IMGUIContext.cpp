...
namespace imgui_display
{
	...
	void IMGUIContext::Init(cacore::IModuleManager* pModuleManager)
	{
		p_RenderBackend = pModuleManager->GetInstance<CRenderBackend>();
		p_WindowSystem = pModuleManager->GetInstance<IWindowSystem>();
	}
	...
	)
	{
		...
		io.Fonts->GetTexDataAsAlpha8(&fontData, &texWidth, &texHeight, &bytesPerPixel);

		m_Fontimage = p_RenderBackend->CreateGPUTexture(GPUTextureDescriptor::Create(texWidth, texHeight
			, ETextureFormat::E_R8_UNORM)
			, ETextureAccessType::eTransferDst | ETextureAccessType::eSampled);;
		m_Fontimage->SetName("FontImage");

		initializeGraph->ScheduleData(m_Fontimage, fontData, texWidth * texHeight * bytesPerPixel);
		initializeGraph->Finalize(m_Fontimage, ETextureAccessType::eSampled);
		IM_FREE(fontData);
		...
		//Register Callbacks
		p_WindowSystem->SetWindowFocusCallback(ImGui_ImplGlfw_WindowFocusCallback);
		p_WindowSystem->SetCursorEnterCallback(ImGui_ImplGlfw_CursorEnterCallback);
		p_WindowSystem->SetCursorPosCallback(ImGui_ImplGlfw_CursorPosCallback);
		p_WindowSystem->SetMouseButtonCallback(ImGui_ImplGlfw_MouseButtonCallback);
		p_WindowSystem->SetScrollCallback(ImGui_ImplGlfw_ScrollCallback);
		p_WindowSystem->SetKeyCallback(ImGui_ImplGlfw_KeyCallback);
		p_WindowSystem->SetCharCallback(ImGui_ImplGlfw_CharCallback);
		p_WindowSystem->SetWindowCloseCallback(ImGui_ImplGlfw_WindowCloseCallback);
		p_WindowSystem->SetWindowPosCallback(ImGui_ImplGlfw_WindowPosCallback);
		p_WindowSystem->SetWindowSizeCallback(ImGui_ImplGlfw_WindowSizeCallback);
	}
	...
	void IMGUIContext::PrepareSingleViewGUIResources(ImGuiViewport* viewPort, GPUGraph* renderGraph)
	{
		...
		shaderArgs->SetSampler(CANAME("IMGUITextureSampler"), TextureSamplerDescriptor::LinearClamp());
		...
		defaultImageArgs->SetImage(CANAME("IMGUITexture"), m_Fontimage
			, GPUTextureView::CreateDefaultForSampling(ETextureFormat::E_R8G8B8A8_UNORM, GPUTextureSwizzle::SingleChannel(EColorChannel::eR)));

		pUserData->m_VertexBuffer = BufferHandle("IMGUI Buffer Handle", true);
		pUserData->m_IndexBuffer = BufferHandle("IMGUI Index Buffer Handle", true);
		renderGraph->AllocBuffer(pUserData->m_VertexBuffer, GPUBufferDescriptor::Create(imDrawData->TotalVtxCount, sizeof(ImDrawVert)));
		renderGraph->AllocBuffer(pUserData->m_IndexBuffer, GPUBufferDescriptor::Create(imDrawData->TotalIdxCount, sizeof(ImDrawIdx)));

		//castl::vector<char> vertexData;
		//castl::vector<char> indexData;
		//vertexData.resize(imDrawData->TotalVtxCount * sizeof(ImDrawVert));
		//indexData.resize(imDrawData->TotalIdxCount * sizeof(ImDrawIdx));
		...
		for (int n = 0; n < imDrawData->CmdListsCount; n++) {
			...
			//memcpy(vertexData.data() + vtxOffset * sizeof(ImDrawVert), cmd_list->VtxBuffer.Data, cmd_list->VtxBuffer.Size * sizeof(ImDrawVert));
			//memcpy(indexData.data() + idxOffset * sizeof(ImDrawIdx), cmd_list->IdxBuffer.Data, cmd_list->IdxBuffer.Size * sizeof(ImDrawIdx));
			...
			for (int j = 0; j < cmd_list->CmdBuffer.Size; ++j)
			{
				...
				if (textureContext != nullptr)
				{
					if (textureContext->m_ViewportRect.width > 0 && textureContext->m_ViewportRect.height > 0)
					{
						...
						textureContext->m_RenderTarget = ImageHandle("ExtraViewport", true);
		...
		//renderGraph->ScheduleData(pUserData->m_VertexBuffer, vertexData.data(), vertexData.size());
		//renderGraph->ScheduleData(pUserData->m_IndexBuffer, indexData.data(), indexData.size());
	}
...
CA_MODULE_INSTANCE(imgui_display::IMGUIContext, imgui_display::IMGUIContext, IMGUIContext);