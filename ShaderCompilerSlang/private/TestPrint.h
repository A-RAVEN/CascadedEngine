...
//
    ...
    static void printf(char const* fmt, Ts ... ts) {
        fprintf(stdout, fmt, ts...);
    }
...
#define SCOPED_OBJECT() //ScopedObject scopedObject##__COUNTER__(this)
    ...
    //    }
            ...
            slang::ParameterCategory layoutUnit)
        {
            printOffset(
                layoutUnit,
                variableLayout->getOffset((SlangParameterCategory)layoutUnit),
                variableLayout->getBindingSpace((SlangParameterCategory)layoutUnit));
        }
        ...
        {
            printSize(layoutUnit, typeLayout->getSize((SlangParameterCategory)layoutUnit));
        }
        ...
        void printKindSpecificInfo(slang::TypeLayoutReflection* typeLayout, AccessPath accessPath)
        {
            ...
            case slang::TypeReflection::Kind::ShaderStorageBuffer:
            {
                ...
                if (containerVarLayout->getTypeLayout()->getSize(
                    (SlangParameterCategory)slang::ParameterCategory::SubElementRegisterSpace) != 0)
                {
            ...
            AccessPath accessPath)
        {
            ...
            result.value += variableLayout->getOffset((SlangParameterCategory)layoutUnit);
            result.space += variableLayout->getBindingSpace((SlangParameterCategory)layoutUnit);
            return result;
        }
            ...
            AccessPath accessPath)
        {
            ...
            default:
                ...
                {
                    result.value += node->variableLayout->getOffset((SlangParameterCategory)layoutUnit);
                }
                break;
            ...
            case slang::ParameterCategory::DescriptorTableSlot:
                ...
                {
                    result.value += node->variableLayout->getOffset((SlangParameterCategory)layoutUnit);
                    result.space += node->variableLayout->getBindingSpace((SlangParameterCategory)layoutUnit);
                }
                for (auto node = accessPath.deepestParameterBlock; node != nullptr; node = node->outer)
                {
                    result.space += node->variableLayout->getOffset((SlangParameterCategory)slang::ParameterCategory::SubElementRegisterSpace);
                }
                break;
            }
            return result;
