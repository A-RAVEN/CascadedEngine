...
namespace catimer
{
	...
	// 32-bit FNV hash
	static uint32_t HashString(const char* pStr)
	{
	...
	{
		uint32_t hash = HashString(pName);
	...
	static void DrawProfilerTimeline(const ImVec2& size = ImVec2(0, 0))
	{
		...
		if (ImGui::ItemAdd(timelineRect, timelineID))
		{
			...
			*/
			...
			auto DrawBar = [&](uint32_t id, uint64_t beginTicks, uint64_t endTicks, uint32_t depth, const char* pName, bool* pOutHovered = nullptr)
				{
					bool hovered = false;
			...
			/*
				(>) Main Thread [1234]
			*/
			auto TrackHeader = [&](const char* pName, uint32_t id)
				{
					...
					else
					{
						...
						if (opacity > 0.0f)
						{
							...
							if (lineStart.x > lineEnd.x)
								castl::swap(lineStart.x, lineEnd.x);
