...
统一「近平面 z-clip / z-clamp」光栅化开关到上层 RenderInterface / GPUGraph PipelineState：`RasterizerStates::enableDepthClamp`（默认 `false` = 标准近平面裁剪），Vulkan 与 D3D12 后端读取同一字段并一致实现其语义（`true` = 近平面沿 z clamp 而非裁剪）。修复 Vulkan 与 D3D12 对近平面外（z<0）几何的裁剪行为不一致，含 Vulkan `depthClamp` feature 启用与 D3D12 `DepthClipEnable` 逆映射（去硬编码）。来源：change unify-depth-clamp-switch（2026-08-30 归档）。
