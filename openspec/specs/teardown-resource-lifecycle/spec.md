# teardown-resource-lifecycle Specification

## Purpose
规定 GPU 后端（vulkan/d3d12）进程 teardown 阶段的资源生命周期安全：设备资源在 `vmaDestroyAllocator` 前全部释放（不触发 VMA 泄漏断言）、通过登记 VMA allocation 实现"扫残留必释放"的兜底、GLFW 窗口单一销毁且回调不访问已释放的对象、崩溃 dump 在 CRT assert/abort 下仍被捕获。
...
窗口（`cawindow::IWindow`）SHALL 至多被销毁一次；`WindowImpl::Release`/自定义 deleter 不得对同一 GLFW 窗口重复 `glfwDestroyWindow`；且窗口生命周期内（含销毁前后）GLFW 派发的回调（focus/size/close 等）不得调用到已释放的窗口系统/回调对象，避免 ACCESS_VIOLATION。
