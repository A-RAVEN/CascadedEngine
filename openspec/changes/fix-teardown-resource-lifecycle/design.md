...
**2026-08-23 复现修正**：d3d12 all-7 `--headless 200` 稳定 exit 1；dump 栈为 `std::_Func_class<IWindow*,bool>::operator()`(functional:930) → `WindowSystem_ImplGlfw_WindowFocusCallback`(WindowSystem_Impl.cpp:67) → `_glfwInputWindowFocus`/`maximizeWindowManually`(win32_window.c:516)。`Rax=0xDDDDDDDDDDDDDDDD` 说明**回调函数对象已释放**。→ 与旧假设（"`~shared_ptr<IWindow>` deleter → `glfwDestroyWindow` 对重复/失效 `m_Window` AV"）**不一致**。真实根因疑似：`s_WindowSystem`（全局单例指针，WindowSystem_Impl.cpp:12）指向的 `WindowSystem` 或其持有的 `m_WindowFocusCallback`（`std::function`）在逆序 teardown 中先于窗口销毁被释放/清空，窗口销毁时 GLFW 派发 window-focus 事件打到已释放回调 → UAF。与 vulkan 逆序 teardown（WindowSystem 排在模块 index 4，逆序销毁早于 IMGUIContext/render backends 释放窗口）相关。**修法需按新根因重新定位**，暂保留幂等 Release 作为兜底，但不以此为主要修复。
