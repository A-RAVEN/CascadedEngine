...
{
	VKShaderStruct::VKShaderStruct(CVulkanApplication& application
		, ShaderCompilerSlang::ShaderStructData const* pStructData)
	...
	void VKShaderStruct::SetValueInternal(cacore::NameHash const& name, void const* pValue, uint32_t sizeInBytes, uint32_t elementIndex)
	{
	...
	void VKShaderStruct::SetStructInternal(cacore::NameHash const& name, castl::shared_ptr<ShaderStruct> const& subStruct, uint32_t elementIndex)
	{
		...
		if (uniformFound != m_NameToUniformElementMetaID.end())
		{
			...
			auto& elementMeta = p_StructData->m_StructUniforms.m_Elements[metaID];
			VKShaderStruct const* pVKSubstruct = static_cast<VKShaderStruct const*>(subStruct.get());
